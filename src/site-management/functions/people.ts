...
import { parseTagsJson } from './utils';
...
export const peopleTools: ToolDefinition[] = [
  defineTool({
    name: 'people.list',
    scope: 'people',
    description: 'List people from source data file. Filter by text query, specific items they use, or tags their items belong to.',
    inputSchema: listPeopleInputSchema,
    handler: async ({ peopleStore, db }, input) => {
      let all = await peopleStore.listPeopleWithSlugs();
